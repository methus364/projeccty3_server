...
// ============================================================
test('createInvoice: รายเดือน เดือนปกติ (พ้น prepaid) → คิดค่าห้องเต็มเดือน + น้ำไฟ, 201', async () => {
  setHandler((sql) => {
    const s = sql.trim();
    if (s.startsWith('BEGIN') || s.startsWith('COMMIT') || s.startsWith('ROLLBACK')) return { rows: [] };
    // โหลด booking — เข้าพัก 1 มิ.ย. (prepaid ครอบถึง 1 ก.ค.)
  ...
  // ออกบิลเดือน ก.ค. = เดือนแรกหลังพ้น prepaid → คิดค่าห้องเต็มเดือน
  const req = { body: { booking_id: 5, month: '2026-07' }, user: { id: 1, role: 'Admin' } };
  const res = makeRes();
  ...
  assert.equal(p[3], 4500);        // room_cost = price_monthly (เต็มเดือน)
  assert.equal(p[4], 400);         // water = 20 × 20
  assert.equal(p[5], 900);         // elec  = 100 × 9
  assert.equal(p[6], 5800);        // total
  assert.ok(calls.some((c) => has(c.sql, 'FOR UPDATE')), 'ต้องล็อกแถว booking ก่อนเช็คบิลซ้ำ กันออกบิลซ้ำตอนมีสอง request พร้อมกัน');
});

test('createInvoice: มิเตอร์ถูกรีเซ็ต (curr < prev) → หน่วยน้ำ/ไฟไม่ติดลบ (นับเป็น 0)', async () => {
  setHandler((sql) => {
    const s = sql.trim();
    if (s.startsWith('BEGIN') || s.startsWith('COMMIT') || s.startsWith('ROLLBACK')) return { rows: [] };
    if (has(s, 'FROM bookings b') && has(s, 'JOIN rooms r') && has(s, 'WHERE b.booking_id')) {
    ...
    if (has(s, 'FROM invoices') && has(s, 'to_char')) return { rows: [] };
    // มิเตอร์ถูกรีเซ็ต: เดือนก่อนน้ำ 9800/ไฟ 5000 → เดือนนี้น้ำ 50/ไฟ 30 (override)
    if (has(s, 'utility_meters')) {
      return { rows: [{ curr_water: 50, prev_water: 9800, curr_elec: 30, prev_elec: 5000 }] };
    }
    if (s.startsWith('INSERT INTO invoices')) return { rows: [{ invoice_id: 78 }] };
    if (has(s, 'FROM invoices i')) return { rows: [fullInvoiceHeader] };
    if (has(s, 'FROM invoice_details')) return { rows: [] };
    return { rows: [] };
  });
  ...
  assert.equal(res.statusCode, 201);
  const p = paramsOf('INSERT INTO invoices');
  assert.equal(p[4], 100);   // water = ขั้นต่ำ (0 หน่วย ยังไม่ถึงขั้นต่ำ WATER_MINIMUM) ไม่ใช่ค่าติดลบ
  assert.equal(p[5], 0);     // elec = 0 หน่วย × เรต = 0 ไม่ใช่ค่าติดลบ
  assert.ok(p[6] >= 4500);   // total ต้องไม่ต่ำกว่าค่าห้องเพียวๆ (พิสูจน์ว่าไม่มีค่าติดลบมาหักออก)
});

test('createInvoice: รายเดือน เดือนแรก (อยู่ในช่วง prepaid) → ค่าห้อง = 0', async () => {
  setHandler((sql) => {
    const s = sql.trim();
    if (s.startsWith('BEGIN') || s.startsWith('COMMIT') || s.startsWith('ROLLBACK')) return { rows: [] };
    if (has(s, 'FROM bookings b') && has(s, 'JOIN rooms r') && has(s, 'WHERE b.booking_id')) {
  ...
  // ออกบิลเดือน มิ.ย. = เดือนเดียวกับเช็คอิน (prepaid ครอบทั้งเดือน) → ค่าห้อง 0
  ...
  assert.equal(res.statusCode, 201);
  const p = paramsOf('INSERT INTO invoices');
  assert.equal(p[3], 0);           // room_cost = 0 (อยู่ในช่วง prepaid)
});
...
test('generateMonthly: มี 1 การจองที่ค้างบิล → ออกบิล 1 ใบ', async () => {
  ...
  assert.ok(calls.some((c) => has(c.sql, 'FOR UPDATE')), 'ต้องล็อกแถว booking ก่อนออกบิล กัน generateMonthly/createInvoice ชนกัน');
});
