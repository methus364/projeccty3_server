...
test('getMeters: คำนวณ diff + ค่าใช้จ่ายถูกต้องตามเรตจริง (น้ำ 10, ไฟ 7)', async () => {
  setHandler((sql) => {
  ...
  assert.equal(row.water_cost, 100);  // 10 หน่วย × 10 บาท
  assert.equal(row.elec_cost, 140);   // 20 หน่วย × 7 บาท
});
