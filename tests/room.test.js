...
test('deleteRoom: มีการจองที่ยังใช้งานอยู่ → 400', async () => {
  setHandler((sql) => {
    ...
    if (has(s, 'FROM rooms') && has(s, 'FOR UPDATE')) return { rows: [{ room_id: 1 }] };
    if (has(s, 'FROM bookings')) return { rows: [{ booking_id: 1 }] };
    return { rows: [] };
  });
  const res = makeRes();
  await room.deleteRoom({ params: { id: 1 } }, res);
  assert.equal(res.statusCode, 400);
  assert.ok(calls.some((c) => has(c.sql, 'ROLLBACK')), 'ต้อง ROLLBACK เมื่อลบไม่ได้');
});
...
test('deleteRoom: ไม่พบห้อง → 404', async () => {
  setHandler((sql) => {
    const s = sql.trim();
    if (s.startsWith('BEGIN') || s.startsWith('COMMIT') || s.startsWith('ROLLBACK')) return { rows: [] };
    if (has(s, 'FROM rooms') && has(s, 'FOR UPDATE')) return { rows: [] }; // ไม่พบห้อง
    return { rows: [] };
  });
  const res = makeRes();
...
test('deleteRoom: ลบสำเร็จ → 200', async () => {
  setHandler((sql) => {
    ...
    if (s.startsWith('BEGIN') || s.startsWith('COMMIT') || s.startsWith('ROLLBACK')) return { rows: [] };
    if (has(s, 'FROM rooms') && has(s, 'FOR UPDATE')) return { rows: [{ room_id: 1 }] };
    if (has(s, 'FROM bookings')) return { rows: [] };
    if (s.startsWith('DELETE FROM rooms')) return { rowCount: 1 };
    return { rows: [] };
  });
  const res = makeRes();
  await room.deleteRoom({ params: { id: 1 } }, res);
  assert.equal(res.statusCode, 200);
  assert.equal(res.body.success, true);
  assert.ok(calls.some((c) => has(c.sql, 'FOR UPDATE')), 'ต้องล็อกแถวห้องก่อนเช็ค+ลบ กัน booking แทรกเข้ามาระหว่างนั้น');
  assert.ok(calls.some((c) => has(c.sql, 'COMMIT')), 'ต้อง COMMIT เมื่อลบสำเร็จ');
});
